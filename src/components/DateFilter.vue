<template>
  <div class="date-filter-wrapper">
    <!-- Min Date Filter -->
    <div class="row mb-2">
      <label class="col-4 col-form-label">Observaties vanaf</label>
      <div class="col-8">
    ...
    <!-- Max Date Filter -->
    <div class="row mb-2">
      <label class="col-4 col-form-label">Observaties tot</label>
      <div class="col-8">
        <div class="date-picker-wrapper">
          <VueDatePicker
            v-model="maxDate"
...
export default {
  ...
  setup() {
    ...
    const absoluteMinDate = new Date(2024, 3, 1); // Minimum toegestane datum voor beide datepickers
    const minDate = ref(defaultDate);
    const maxDate = ref(null); // Einddatum blijft initieel null

    // Computed property voor de minimum datum van de maxDate picker
    const minDateForMaxPicker = computed(() => {
      if (minDate.value && minDate.value > absoluteMinDate) {
        return minDate.value;
      }
      return absoluteMinDate;
    });
...
/* Customize vue3-datepicker styles to match the form controls */
:deep(.dp__input) {
