    ...
    <div v-if="selectedObservation">
        ...
        <div class="container mt-5">
            ...
            <div class="d-flex justify-content-between mb-3" id="reservation">
                <button v-if="canReserve && !selectedObservation.reserved_by" class="btn btn-sm btn-outline-primary" @click="reserveObservation">Reserveren</button>
                <span v-if="selectedObservation.reserved_by">Gereserveerd door {{ selectedObservation.reserved_by_first_name }} (nog {{ reservationStatus }})</span>
                <button v-if="(isUserReserver || canEditAdminFields) && selectedObservation.reserved_by" class="btn btn-sm btn-outline-danger" @click="cancelReservation">Reservatie annuleren</button>
            </div>
            ...
            <div class="accordion accordion-flush mb-3" id="sections">
                <section class="accordion-item">
                    <h4 class="accordion-header" id="eradication-header">
                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#eradication" aria-expanded="false" aria-controls="eradication">
                            <strong>Bestrijding</strong>
                            <span class="badge bg-danger ms-2">{{ selectedObservation.eradication_datetime ? 'Bestreden' : 'Niet bestreden' }}</span>
                        </button>
                    </h4>
                    <div id="eradication" class="accordion-collapse collapse" aria-labelledby="eradication-header" data-bs-parent="#sections">
                        <div class="accordion-body">
                            <div class="row mb-2">
                                <label class="col-4 col-form-label">Datum</label>
                                <div class="col-8">
                                    <input v-if="selectedObservation.eradication_datetime !== undefined" v-model="editableObservation.eradication_datetime" type="datetime-local" class="form-control" :readonly="!canEdit" :class="{ 'form-control-plaintext': !canEdit }" />
                                </div>
                            </div>
                            <div class="row mb-2">
                                <label class="col-4 col-form-label">Uitvoerder</label>
                                <div class="col-8">
                                    <input v-if="selectedObservation.eradicator_name !== undefined" v-model="editableObservation.eradicator_name" type="text" class="form-control" :readonly="!canEdit" :class="{ 'form-control-plaintext': !canEdit }" />
                                </div>
                            </div>
                            <div class="row mb-2">
                                <label class="col-4 col-form-label">Duur</label>
                                <div class="col-8">
                                    <input v-if="selectedObservation.eradication_duration !== undefined" v-model="editableObservation.eradication_duration" type="text" class="form-control" :readonly="!canEdit" :class="{ 'form-control-plaintext': !canEdit }" />
                                </div>
                            </div>
                            <div class="row mb-2">
                                <label class="col-4 col-form-label">Personeel</label>
                                <div class="col-8">
                                    <input v-if="selectedObservation.eradication_persons !== undefined" v-model="editableObservation.eradication_persons" type="number" class="form-control" :readonly="!canEdit" :class="{ 'form-control-plaintext': !canEdit }" />
                                </div>
                            </div>
                            <div class="row mb-2">
                                <label class="col-4 col-form-label">Resultaat</label>
                                <div class="col-8">
                                    <select v-if="selectedObservation.eradication_result !== undefined" v-model="editableObservation.eradication_result" class="form-select" :disabled="!canEdit">
                                        <option v-for="(label, value) in eradicationResultEnum" :key="value" :value="value">{{ label }}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row mb-2">
                                ...
                                <div class="col-8">
                                    <select v-if="selectedObservation.eradication_method !== undefined" v-model="editableObservation.eradication_method" class="form-select" :disabled="!canEdit">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-8">
                                    <select v-if="selectedObservation.eradication_product !== undefined" v-model="editableObservation.eradication_product" class="form-select" :disabled="!canEdit">
                                        <option v-for="(label, value) in eradicationProductEnum" :key="value" :value="value">{{ label }}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row mb-2">
                                ...
                                <div class="col-8">
                                    <select v-if="selectedObservation.eradication_aftercare !== undefined" v-model="editableObservation.eradication_aftercare" class="form-select" :disabled="!canEdit">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-8">
                                    <select v-if="selectedObservation.eradication_problems !== undefined" v-model="editableObservation.eradication_problems" class="form-select" :disabled="!canEdit">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-8">
                                    <textarea v-if="selectedObservation.eradication_notes !== undefined" v-model="editableObservation.eradication_notes" rows="2" class="form-control" :readonly="!canEdit" :class="{ 'form-control-plaintext': !canEdit }"></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <section class="accordion-item">
                    <h4 class="accordion-header" id="nest-header">
                        <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#nest" aria-expanded="true" aria-controls="nest">
                            <strong>Nest info</strong>
                        </button>
                    </h4>
                    <div id="nest" class="accordion-collapse collapse show" aria-labelledby="nest-header" data-bs-parent="#sections">
                        <div class="accordion-body">
                            <div v-if="selectedObservation.images && selectedObservation.images.length > 0" id="carousel-12040" class="carousel carousel-dark slide carousel-fade mb-2" data-bs-ride="carousel" data-bs-keyboard="false" data-bs-interval="500">
                                <div class="carousel-indicators">
                                    <button v-for="(image, index) in selectedObservation.images" :key="index" type="button" :data-bs-target="'#carousel-12040'" :data-bs-slide-to="index" :class="{ active: index === 0 }" :aria-current="index === 0 ? 'true' : undefined" :aria-label="'Foto ' + (index + 1)"></button>
                                </div>
                                <div class="carousel-inner">
                                    <div class="carousel-item d-flex w-100 justify-content-center bg-light" v-for="(image, index) in selectedObservation.images" :key="index" :class="{ active: index === 0 }">
                                        <img style="max-height: 200px;" :src="image">
                                    </div>
                                </div>
                                <button class="carousel-control-prev" type="button" data-bs-target="#carousel-12040" data-bs-slide="prev">
                                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                    <span class="visually-hidden">Vorige</span>
                                </button>
                                <button class="carousel-control-next" type="button" data-bs-target="#carousel-12040" data-bs-slide="next">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-8">
                                    <select v-if="selectedObservation.nest_location !== undefined" v-model="editableObservation.nest_location" class="form-select" :disabled="!canEdit">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-8">
                                    <select v-if="selectedObservation.nest_size !== undefined" v-model="editableObservation.nest_size" class="form-select" :disabled="!canEdit">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-8">
                                    <select v-if="selectedObservation.nest_height !== undefined" v-model="editableObservation.nest_height" class="form-select" :disabled="!canEdit">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-8">
                                    <p class="form-control-plaintext"><a :href="'https://waarnemingen.be/observation/' + selectedObservation.wn_id" target="_blank">Waarnemingen.be</a></p>
                                </div>
                            </div>
                            <div class="row mb-2">
                ...
                <section class="accordion-item" v-if="canViewContactInfo">
                    ...
                    <div id="contact" class="accordion-collapse collapse" aria-labelledby="contact-header" data-bs-parent="#sections">
                        <div class="accordion-body">
                            <div class="row mb-2">
                ...
                <section class="accordion-item" v-if="canEditAdminFields">
                    ...
                    <div id="admin" class="accordion-collapse collapse" aria-labelledby="admin-header" data-bs-parent="#sections">
                        <div class="accordion-body">
                            <div class="row mb-2">
                                <div class="col-8 offset-4">
                                    <div class="form-check form-switch">
                                        <input v-if="selectedObservation.visible !== undefined" v-model="editableObservation.visible" class="form-check-input" type="checkbox" id="visible" :disabled="!canEdit" />
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-8">
                                    <input v-if="selectedObservation.wn_cluster_id !== undefined" v-model="editableObservation.wn_cluster_id" type="text" class="form-control" :readonly="!canEdit" :class="{ 'form-control-plaintext': !canEdit }" />
                                </div>
                            </div>
                            <div class="row mb-2">
                                <div class="col-8 offset-4">
                                    <div class="form-check form-switch">
                                        <input v-if="selectedObservation.observer_received_email !== undefined" v-model="editableObservation.observer_received_email" class="form-check-input" type="checkbox" id="observer-got-email" :disabled="!canEdit" />
                                        <label class="form-check-label" for="observer-got-email">Melder kreeg e-mail</label>
                                    </div>
                                </div>
                            </div>
                            <div class="row mb-2">
                                <div class="col-8 offset-4">
                                    <div class="form-check form-switch">
                                        <input v-if="selectedObservation.public_domain !== undefined" v-model="editableObservation.public_domain" class="form-check-input" type="checkbox" id="public-domain" :disabled="!canEdit" />
                            ...
                            <div class="row mb-2">
                                <label class="col-4 col-form-label">Opmerkingen</label>
                                <div class="col-8">
                                    <textarea v-if="selectedObservation.admin_notes !== undefined" v-model="editableObservation.admin_notes" rows="2" class="form-control" :readonly="!canEdit" :class="{ 'form-control-plaintext': !canEdit }"></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <p class="mb-3 text-muted small" id="metadata">
                Aangemaakt op <span class="created-datetime">{{ selectedObservation.created_datetime ? formatDate(selectedObservation.created_datetime) : '' }}</span> door <span class="created-by">{{ selectedObservation.created_by_first_name || '' }}</span>, gewijzigd op <span class="modified-datetime">{{ selectedObservation.modified_datetime ? formatDate(selectedObservation.modified_datetime) : '' }}</span> door <span class="modified-by">{{ selectedObservation.modified_by_first_name || '' }}</span>.
            </p>
        </div>
    </div>
</template>
